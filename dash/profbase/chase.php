...
if(isset($_GET['call']) && !empty($_GET['call'])){
$call = xclean($_GET['call']);
$query="SELECT id,pid,project_owner,timereal,token FROM projects WHERE bidder_email='$user' AND status='approved' AND pid LIKE '%$call%' ORDER BY id desc
...
<caption class="capp"><i class="fa fa-briefcase"></i> Total <font class='coml'>Client = <?php echo x_count("bidded","bidder_email='$user' AND status='approved'");?></font></caption>
...
<?php
	...
	foreach(x_select("user_photo","userdb","email='$po' AND status='active'","1","name") as $key){
	$photo = $key["user_photo"];
	}
	
	foreach(x_select("ptitle,amount_to_pay,amount_currency,time_from,time_to,payment_status,token","projects","id='$pid'","1","id") as $key){
	$ptitle = $key["ptitle"];
...
<button onclick="<?php
if($ps == "active"){
	?>	load('profbase/chatme?tid=<?php echo $pid;?>&key=<?php echo $ptoken;?>&po=<?php echo $po;?>&pt=<?php echo $ptitle;?>')<?php
	}else{
		?>alert('Sorry! Project owner must pay before chatting!')<?php
		}
?>" class="btn btn-success"><i class="fa fa-comment"></i> chat 
<?php 
$user = xclean($_SESSION["PBNG_EMAIL_2018_VISION"]);
$cont = x_count("chatbox","pid='$pid' AND ptoken='$ptoken' AND email !='$user' AND seen='0'");
if($cont > 0){
...
$msg .= "<p class='text-center'>No Job approved yet!!</p>";
			echo $msg;

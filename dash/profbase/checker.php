...
if(isset($_GET['key']) && isset($_GET['tid']) && !empty($_GET['key']) && !empty($_GET['tid'])){
	...
	$token = xg("key");
	$email = xg("tid");
	if(x_count("projects","id='$email' AND token='$token' AND processing_status='active' LIMIT 1") > 0){
		foreach(x_select("0","projects","id='$email' AND token='$token' AND processing_status='active'","1","id") as $key ){
		$id = $key["id"];
		$title = $key["ptitle"];
		$status = $key["status"];
		$des = $key["pdes"];
	...
	foreach(x_select("user_photo,name,gender,last_login_r,realtime","userdb","email='$user' AND status='active'","1","name") as $key){
	...
	$gen = $key["gender"];
		...
		?>
<button onclick="load('profbase/apprjob')" style="padding:15px;width:250px;margin-bottom:20pt;" class="btn btn-success"><i class="glyphicon glyphicon-check"></i> APPROVED JOBS &nbsp;&nbsp;&nbsp;&nbsp;<span class="badge"><?php

if(isset($_SESSION["PBNG_EMAIL_2018_VISION"])){
	$usere = xclean($_SESSION["PBNG_EMAIL_2018_VISION"]);
	echo x_count("projects","processing_status='active' AND approved_to='$usere'");
}else{
	echo "0";
}
...
?></span></button>
		...
		<th> <?php
if($pys == "active"){
	?><img src="../<?php echo $photo;?>" class="img-responsive remitrr"/><?php
	}else{
		?><img src="../image/avatar.png" class="img-responsive remitrr"/><?php
...
if($pys == "active"){
	...
	<tr>
	<th>NAME</th>
	<td><?php echo htmlspecialchars(ucfirst(strtoupper($pname)));?></td>
	</tr>
	
	<tr>
	<th>GENDER</th>
	<td><?php echo htmlspecialchars(ucfirst(strtoupper($gen)));?></td>
	</tr>
	
	<tr>
	<th>JOINED ON</th>
	<td><?php echo htmlspecialchars(ucfirst(strtoupper($jd)));?></td>
	</tr>
	
	<tr>
	<th>LAST LOGIN</th>
	<td><?php echo htmlspecialchars(ucfirst(strtoupper($ll)));?></td>
	</tr>
...
<?php
			...
			echo "No user found!";
				...
				?>
<tr><th><i class="fa fa-phone"></i>&nbsp;&nbsp;&nbsp; MOBILE</th><td>Client not paid</td></tr>

<tr><th><i class="fa fa-inbox"></i>&nbsp;&nbsp;&nbsp; EMAIL</th><td>Client not paid</td></tr>
				<?php
		...
		echo "Project is not approved to someone";
	}

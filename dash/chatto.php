...
$query="SELECT id,ptitle,status,pdes,timereal,amount_to_pay,amount_currency,owner,pcategory,time_from,time_to,token,payment_status FROM projects WHERE owner='$user' AND  status='active' AND bidded_status='active' AND processing_status='active' AND completion_status='inactive' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
...
<tr><th>No</th><th>Project Title</th><th>Payment status</th><th>Action</th></tr>
<?php
	...
	foreach(x_select("name","userdb","email='$user' AND status='active'","1","name") as $key){
	$namee = $key["name"];
	}
...
<td><?php echo ucfirst(strtolower($title));?></td>
<!---<td><?php echo $amt_c." ".number_format($amt,0)?></td>--->
<!--<td><?php echo $status?></td>--->
<td><font style='color:blue;'><?php echo $pay?></font></td>
<td>
<button onclick="<?php
if($pay == "active"){
	?>	load('../dash/profbase/chatme?tid=<?php echo $id;?>&key=<?php echo $token;?>&po=<?php echo $user;?>&pt=<?php echo $title;?>&nm=<?php echo $namee;?>')<?php
	}else{
		?>alert('Sorry! Project owner must pay before chatting!')<?php
		}
?>" class="btn btn-primary"><i class="fa fa-comment"></i> Chat 
	<?php 
$user = xclean($_SESSION["PBNG_EMAIL_2018_VISION"]);
$cont = x_count("chatbox","pid='$id' AND ptoken='$token' AND email !='$user' AND seen='0'");
if($cont > 0){

?>&nbsp;<span class="badge"><?php echo $cont." ";?>unread</span><?php
	}else{
...
$msg = "<p class='text-center' style='font-size:60pt;margin-bottom:10pt;'><span class='fa fa-comment'></span></p>";
$msg .= "<p class='text-center'>No active chat!</p>";
			echo $msg;

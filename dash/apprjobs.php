...
<caption class="capp"><i class="fa fa-briefcase"></i> Total <font class='coml'>Approved = <?php echo x_count("projects","owner='$user' AND  status='active' AND bidded_status='active' AND processing_status='active' AND completion_status='inactive'");?></font></caption>
<tr><th>No</th><th>Project Title</th><th>Budget amount</th><th>Status</th><th>Payment</th><th>Action</th></tr>
<?php
...
<td><?php echo $pay?></td>
<td>
<button onclick="load('checkappr?tid=<?php echo $id;?>&key=<?php echo $token;?>')" class="btn btn-primary"><i class="fa fa-edit"></i></button>&nbsp;&nbsp;
<?php
if(x_count("bidded","pid='$id' AND bidder_email='$appr_to' AND status='approved' LIMIT 1") > 0){
	
	foreach(x_select("id","bidded","pid='$id' AND bidder_email='$appr_to' AND status='approved'","1","id") as $key){
		$idp = $key["id"];
		?><button onclick="load('check_prof?pid=<?php echo $id;?>&token=<?php echo $token;?>&userkey=<?php echo $idp;?>&bidmail=<?php echo $appr_to;?>&loc=approved')" class="btn btn-success"><i class="fa fa-user"></i> Prof. info</button><?php
		}

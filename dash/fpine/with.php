...
$query="select id from withdrawalbase WHERE status='pending' AND type='withdrawal' AND email LIKE '%$call%' order by id desc";
}else{
$query="select id from withdrawalbase WHERE status='pending' AND type='withdrawal' order by id desc";
}
...
if(isset($_GET['call']) && !empty($_GET['call'])){
$call = xclean($_GET['call']);
$query="SELECT * FROM withdrawalbase WHERE status='pending' AND type='withdrawal' AND email LIKE '%$call%' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}else{
$query="SELECT * FROM withdrawalbase WHERE status='pending' AND type='withdrawal' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
...
$msg .= "<p class='text-center'>No Pending Transaction!</p>";
			echo $msg;

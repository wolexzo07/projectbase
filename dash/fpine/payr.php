...
$query="select id from projects WHERE status='inactive' AND ptitle LIKE '%$call%' order by id desc";
}else{
$query="select id from projects WHERE status='inactive' order by id desc";
}
...
if(isset($_GET['call']) && !empty($_GET['call'])){
$call = xclean($_GET['call']);
$query="SELECT processing_status,id,ptitle,status,pdes,timereal,amount_to_pay,amount_currency,owner,pcategory,time_from,time_to,token FROM projects WHERE status='inactive' AND ptitle LIKE '%$call%' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}else{
$query="SELECT processing_status,id,ptitle,status,pdes,timereal,amount_to_pay,amount_currency,owner,pcategory,time_from,time_to,token FROM projects WHERE status='inactive' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
...
<td>
	...
	<button class="btn btn-warning"><i class="glyphicon glyphicon-check"></i></button>
	</form>
...
<button style="float:left;margin-left:5pt;" onclick="load('checkfull?tid=<?php echo $id;?>&key=<?php echo $token;?>')" class="btn btn-primary"><i class="fa fa-edit"></i></button>
<button style="float:left;margin-left:5pt;" class="btn btn-success"><i class="fa fa-trash"></i></button>
<br/><br/>
...
$msg .= "<p class='text-center'>No Pending Job!</p>";
			echo $msg;

...
$query="select id from projects WHERE status='active' AND processing_status='active' AND title LIKE '%$call%' order by id desc";
}else{
$query="select id from projects WHERE status='active' AND processing_status='active' order by id desc";
}
...
if(isset($_GET['call']) && !empty($_GET['call'])){
$call = xclean($_GET['call']);
$query="SELECT processing_status,id,ptitle,status,pdes,timereal,amount_to_pay,amount_currency,owner,pcategory,time_from,time_to,token FROM projects WHERE status='active' AND processing_status='active' AND ptitle LIKE '%$call%' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}else{
$query="SELECT processing_status,id,ptitle,status,pdes,timereal,amount_to_pay,amount_currency,owner,pcategory,time_from,time_to,token FROM projects WHERE status='active' AND processing_status='active' ORDER BY id desc
limit ".mysqli_real_escape_string(x_cstring(),$start).",$recordsPerPage";
}
